...
# Converted to compatible with both of python 2.x and 3.x versions by M. B. Ghaznavi-Ghoushchi
    ...
    def do_POST(self):
        ...
        if ctype == 'multipart/form-data':
            for k, v in cgi.parse_multipart(self.rfile, pdict).items():
                ...
                if type(k) == bytes:
                    k = k.decode()
                postvars[k] = [s.decode() if type(
                    s) == bytes else s for s in v]
        ...
        try:
            ...
            self.wfile.write(response.decode().encode('utf-8'))
